...
R::ext('seed', function ($beanType, $number, $fields) use ($seeder) {
    return $seeder->seed($beanType, $number, $fields);
});
R::ext('unseed', function ($beanType) use ($seeder) {
    return $seeder->unseed($beanType);
});
R::ext('setSeederHashMethod', function ($algorithm) use ($seeder) {
    return $seeder->setHashMethod($algorithm);
});
